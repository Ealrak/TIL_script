...
function storyFunc1() {
    let person = ['헨젤','그레텔'];
    let src = '조약돌';
    //헨젤과 그레텔이 조약돌을 이용해서 집을 찾아갔다.
    console.log(`${person[0]}과 ${person[1]}이 ${src}을 이용해서 집을 찾아갔다.`)
}
function storyFunc2() {
    let person = ['헨젤','그레텔'];
    let src = '빵부스러기';
    //헨젤과 그레텔이 빵부스러기를 이용해서 집을 찾아갔다.
    console.log(`${person[0]}과 ${person[1]}이 ${src}를 이용해서 집을 찾아갔다.`)
}
storyFunc1(); //함수호출
storyFunc2(); //함수호출
...
function storyFunc(src) {
    let person = ['헨젤','그레텔'];
    //let src = '빵부스러기';
    //헨젤과 그레텔이 조약돌을(를) 이용해서 집을 찾아갔다.
    //헨젤과 그레텔이 빵부스러기을(를) 이용해서 집을 찾아갔다.
    console.log(`${person[0]}과 ${person[1]}이 ${src}을(를) 이용해서 집을 찾아갔다.`)
}
storyFunc('조약돌');
storyFunc('빵부스러기');
console.log('--------------------------------------커피 레시피(기본값 포함)')
//function coffeeFunc1(){} 함수를 생성한다. 실행결과가 항상 같으므로 매개변수는 선언하지 않는다.
//function coffeeFunc(data1){} 함수를 생겅한다. 결과 실행 시 매번 달라지는 데이터가 1개 있기 때문에 매개변수를 1개 생성한다.
//function coffeeFunc(data1, data2){} 함수를 생겅한다. 결과 실행 시 매번 달라지는 데이터가 2개 있기 때문에 매개변수를 2개 생성한다.
//function coffeeFunc(data1){}
//coffeeFunc1(10);
//(위)프로그래밍 해석 : 함수를 생성 후 달라지는 데이터1개에 맞춘 매개변수 1개 생성완료. 이후 함수를 호출하고 매개변수에 대입할 10값 입력.
...
coffeeFunc1(2, 1);
coffeeFunc1(3, 0.5);
coffeeFunc1(); //매개변수 요청하지 않으면 기본값으로 나온다.
//=====================커피주문 알고리즘(고객입장)
...
coffeeKiosk('아메리카노',2);//기본값이라 두번째 매개변수 생략
coffeeKiosk('녹차라떼');
coffeeKiosk('아메리카노',4);
console.log('-----------------------------')
...
plusFunc(2);
//호출 할 때 리턴이 보낸 반환값을 묶어주는 실행함수를 별도로 작성한다.
