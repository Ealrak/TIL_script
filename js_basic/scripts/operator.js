...
//증감연산은 x변수 자체에 영향을 주기 때문에 일회성 더하기 연산자으로 +1을 붙여서 계산하고 x에 영향을 주지 않게 y에 대입한다.
let y = x+1;
console.log(x, y); //5, 6
y = x++;
console.log(x, y); //6, 5
...
number *= 5; //곱하기복합
console.log(number); //50 number = number*5;
number /= 5; //나누기복합
console.log(number); //10 number = number/5;
number %= 5; //나머지복합
console.log(number); //0 number = number%5;