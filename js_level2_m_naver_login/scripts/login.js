...
//변수생성 -> 이벤트제작 -> 이벤트 만족 시 실행함수 (중간중간 콘솔 자주체크)
...
조건3 결과) 아이디(로그인 전화번호, 로그인 전용 아이디) 또는 비밀번호 입력정보가 잘못되었습니다. 아이디와 비밀번호를 정확히 입력해 주세요.
조건4) 조건3 상황에서 로그인실패가 3회 이상일 때
조건4 결괴) 영수증 퀴즈와 '아이디(로그인 전화번호, 로그인 전용 아이디), 비밀번호 또는 자동입력 방지 문자롤 잘못 입력했습니다. 입력하신 내용을 다시 확인해주세요.' 메세지 출력
*/
