...
// 2. 우측 ...메뉴 클릭 시 저장~신고 등 기능 실행 (o) - 저장~신고 제작 필요
// 3. 사진 한번 터치 시 반응 없음 (x js)
// 4. 사진 한번 터치 시 좋아요 자동 등록 (o)
...
//익명함수 또는 화살표함수로 작성 시 이벤트 앞 객체정보가 함수의 매개변수 정보로 자동으로 대입된다.
...
// 6. 댓글 아이콘 터치 시 댓글 입력창 실행 (o) - 제작 필요, 실행화면에서 바깥쪽 영역 터치 댓글입력창 닫힘 (o) - 제작 필요
// 7. 공유 아이콘 터치 시 공유가능 계정들 표시된 창 실행 (o) - 제작 필요, 바깥쪽 영역 터치 시 닫힘 (o)
// 8. 즐겨찾기 아이콘 터치 시 즐겨찾기 저장(x - DB필요)이 됨과 동시에 컬렉션 선택 화면 출력(o), 바깥쪽 영역 터치 시 닫힘
// 9. 글 1줄, 날짜만 미리표시, 글 터치 시 전체 내용(나머지 글 줄과 해시태그 포함)펼치기 (o)
// 글 1줄, 날짜만 미리표시

const stroyDOM = document.querySelector('.story');
const dateDOM = document.querySelector('.date');
const tagDOM = document.querySelector('.tag');
console.log(stroyDOM, dateDOM);
//함수를 생성하지 않고 바로 작성한 이유 - 반복이 아니고 즉시 실행되기때문
tagDOM.style.display = 'none';
//글 클릭 시 태그 표시
stroyDOM.addEventListener('click',tagShow)
function tagShow(){
