...
// 12-2. 위 (12)번 색상 -> 사이즈 선택 시 개로운 요소(.order_select) 보이기
...
function orderFunc(){
    ...
    if(colorSelect.value && sizeSelect.value){
        ...
        //size_select의 color안에 선택한 sizeSelect 값 대입하기
...
//초기 설정
orderNum.value = num;
//증가 +버튼 클릭 시 수량이 1씩 증가되서 orderNum출력
plusBtn.addEventListener('click',()=>{
    num++;
    if(num > 9999){alert('1~9999개까지 선택가능합니다.')}
    if(num >= 1 && num <= 9999){ //1~9999개
        total = num*price;
        orderNum.value = num;
        totalPrice.textContent = total.toLocaleString('ko-kr');//toLocaleString - 국가의 숫자표기법 처럼 표시하는 명령어 예) ko-kr : 한국 (,) 표시 fi-fi : 핀란드 ( )표시
    }
})
//감소 -버튼 클릭 시 수량이 1씩 감소되서 orderNum출력
minusBtn.addEventListener('click',()=>{
    num--;
    //감소 수량이 1보다 작은 경우 경고메세지 출력
    if(num < 1){alert('1~9999개 까지 선택가능합니다.')}
    if(num >= 1){
        total = num*price;
        orderNum.value = num;
